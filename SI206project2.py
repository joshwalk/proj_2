...
def grab_headlines():
    ...
    links = most_read_div.find_all('a') # finds links in view-most-read class
    for link in links:
        headline_list.append(link.text.strip()) # adds text of link to headline_list
...
def get_umsi_data():
    ...
    while count <= 12:
        ...
        # finds div w/ specific property and gets text of h2
        name_div = soup.find_all("div", {"property" : "dc:title"})
        for names in name_div:
            name_list.append(names.h2.text) # adds each instance to name list
    ...
    umsi_titles = dict(zip(name_list,title_list)) # builds a dict: keys are from name_list, values from title_list
    return umsi_titles
...
def num_students(data):
    ...
    for value in data.values():
        if value == "PhD student": # finds values that match this string
            phd_count += 1 # increments by 1 each time it's found
